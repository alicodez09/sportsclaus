...
export default function EditModal({
    ...
    useEffect(() => {
        ...
        fields.forEach((field) => {
            // Skip country fields
            if (field.name.startsWith("country")) return

            // Handle nested objects (excluding country)
            if (field.name.includes(".")) {
                const parts = field.name.split(".")
                initialData[field.name] = data[parts[0]]?.[parts[1]] || ""
            } else {
    ...
    const validateForm = () => {
        ...
        fields.forEach((field) => {
            // Skip validation for country fields
            if (field.name.startsWith("country")) return
        ...
        if (validateForm()) {
            try {
                ...
                fields.forEach((field) => {
                    ...
                    if (field.name.includes(".")) {
                        // Handle nested fields (excluding country)
                        const parts = field.name.split(".")
                        if (!apiData[parts[0]]) apiData[parts[0]] = {}
                        apiData[parts[0]][parts[1]] = formData[field.name]
                    } else {
                        apiData[field.name] = formData[field.name]
                    }
                })

                // Make sure to include the _id
                ...
                setErrors({
                    submit: "Failed to update player. Please try again.",
                })
            } finally {
            ...
            <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="space-y-4">
                        {fields
                            .filter(
                                (field) => !field.name.startsWith("country"),
                            ) // Filter out country fields
                            .map((field) => (
