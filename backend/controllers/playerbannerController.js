import playerbannerModel from "../models/playerbannerModel.js"

export const Create = async (req, res) => {
  console.log(req.body, "req.body")
  const result = await playerbannerModel.create(req.body);
  if (result) {
    res.status(201).send({
      success: true,
...
export const Get = async (req, res) => {
  const result = await playerbannerModel.find();
  if (result) {
    res.status(201).send({
      success: true,
...
export const Update = async (req, res) => {
  try {
    ...
    const updatedNewsfeed = await playerbannerModel.findByIdAndUpdate(
      id,
      updatedData,
      {
        new: true,
...
export const Delete = async (req, res) => {
  try {
    ...
    const deleteNewsfeed = await playerbannerModel.findByIdAndDelete(id);
