import latestnewsModel from "../models/latestnewsModel.js";

export const Create = async (req, res) => {
  const result = await latestnewsModel.create(req.body);
  if (result) {
    res.status(201).send({
      success: true,
...
export const Update = async (req, res) => {
  try {
    ...
    const updatedNewsfeed = await latestnewsModel.findByIdAndUpdate(
      id,
      updatedData,
      {
        new: true,
...
export const Delete = async (req, res) => {
  try {
    ...
    const deleteNewsfeed = await latestnewsModel.findByIdAndDelete(id);
