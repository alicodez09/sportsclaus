import playerrankingModel from "../models/playerrankingModel.js"
import countriesModel from "../models/countries.js"


export const Create = async (req, res) => {
  console.log(req.body, "req.body")
  const result = await playerrankingModel.create(req.body);
  if (result) {
    res.status(201).send({
      success: true,
...
export const Get = async (req, res) => {
  const result = await playerrankingModel.find();
  if (result) {
    res.status(201).send({
      success: true,
...
export const Update = async (req, res) => {
  try {
    ...
    const updatedNewsfeed = await playerrankingModel.findByIdAndUpdate(
      id,
      updatedData,
      {
        new: true,
...
export const Delete = async (req, res) => {
  try {
    ...
    const deleteNewsfeed = await playerrankingModel.findByIdAndDelete(id);
